...
execute anchored eyes positioned ^ ^ ^ positioned ~-1.5 ~-1.5 ~-1.5 align xyz as @e[dx=2,dy=2,dz=2,type=!player,tag=!delta.launcher,tag=!global.ignore] at @s run function delta:internal/subtick/shift_entity

#Put all other players that may be hit by the explosion into spectator mode
tag @s add delta.ignore
execute anchored eyes positioned ^ ^ ^ positioned ~-1.5 ~-1.5 ~-1.5 align xyz as @a[dx=2,dy=2,dz=2,tag=!delta.ignore] at @s run function delta:internal/subtick/ignore_player
